...
function hydrateRequest(context, request) {
    ...
    hydrated.className = helpers_2.ensureSuffix(helpers_1.pascalCase(request.name), helpers_1.upperCaseFirst(suffix));
    hydrated.fileName = helpers_2.removeSuffix(helpers_1.paramCase(request.name), "-" + helpers_1.paramCase(suffix));
...
function hydrateTabRequest(context, request) {
    ...
    for (var i = 0; i < request.tabs.length; i++) {
        var tabVar = helpers_1.camelCase(request.tabs[i].name) + "Root";
        ...
        if (i === request.tabs.length - 1) {
            hydrated.tabContent += "    <ion-tab [root]=\"" + tabVar + "\" tabTitle=\"" + helpers_1.sentenceCase(request.tabs[i].name) + "\" tabIcon=\"information-circle\"></ion-tab>";
        }
        else {
            hydrated.tabContent += "    <ion-tab [root]=\"" + tabVar + "\" tabTitle=\"" + helpers_1.sentenceCase(request.tabs[i].name) + "\" tabIcon=\"information-circle\"></ion-tab>\n";
        }
...
function nonPageFileManipulation(context, name, ngModulePath, type) {
    ...
    return helpers_1.readFileAsync(ngModulePath).then(function (content) {
        ...
        fileContent = typescript_utils_1.insertNamedImportIfNeeded(ngModulePath, fileContent, hydratedRequest.className, path_1.relative(path_1.dirname(ngModulePath), hydratedRequest.dirToWrite) + "/" + hydratedRequest.fileName);
        if (type === 'provider') {
